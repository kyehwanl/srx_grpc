...
func worker(jobChan <-chan Job, workerId int32) {
	...
	for job := range jobChan {
		fmt.Printf("+++ [worker] (id: %d) job channel received : %#v\n", workerId, job)
		fmt.Println("+++ start Proxy Verify")
		ProxyVerify(job.data, job.grpcClientID, job.done, workerId)
		fmt.Println("+++ Finished Proxy Verify ")
		fmt.Println("+++ ... Waiting for the next job channel .... ")
	...
	fmt.Printf("+++ worker gorouting closed \n ")
}
...
func InitWorkerPool() bool {
	...
	fmt.Printf("+++ [InitWorkerPool] go worker pool generating as many as worker counter: %d \n ", WorkerCount)
	...
	wg.Wait()
	fmt.Printf("+++ worker pool closed -- NEED TO REGEN \n ")
...
func RunProxyHello(data []byte) (*C.uchar, uint32) {
	...
	fmt.Println("+ RunProxyHello in srx_grpc_server.go called ")
	cli := client.cli
	fmt.Println()
	fmt.Printf("+ client : %#v\n", client)
	fmt.Printf("+ client.cli : %#v\n", client.cli)
	...
	*/
	...
	fmt.Println("+ Trying to call cli. proxy hello through protocol buffer ")
...
func RunProxyVerify(data []byte, grpcClientID uint32) uint32 {
	...
	fmt.Printf("++ [RunProxy Verify] data: %#v, clientID: %d\n", data, grpcClientID)
	job := NewJob(data, grpcClientID)
	fmt.Printf("++ New job generated: %#v\n", job)
...
func ProxyVerify(data []byte, grpcClientID uint32, jobDone chan bool, workerId int32) uint32 {
	...
	fmt.Printf("[WorkerID: %d] ProxyVerify Update Count : %d\n", workerId, g_count)
	if g_count == 0 {
		start = time.Now()
	}
	g_count++
	...
	fmt.Printf("input data for Proxy Verify Stream: %#v\n", data)
	...
	fmt.Printf("[WorkerID: %d] stream info: %#v\n", workerId, stream)
	...
	go func(stream pb.SRxApi_ProxyVerifyStreamClient, jobDone chan bool, workerId int32) {
		...
		for {
			...
			fmt.Printf("[WorkerID: %d] (inside go routine, after stream Recv) stream: %#v\n", workerId, stream)

			fmt.Println()
			fmt.Printf("[WorkerID: %d, update count: %d] response Notify data : %#v\n", workerId, g_count, resp)
			fmt.Printf("[WorkerID: %d] size : %#v\n", workerId, resp.Length)

			if resp.Type == 0 && resp.Length == 0 {
				_, _, line, _ := runtime.Caller(0)
				log.Printf("[Proxy_Verify][WorkerID: %d] [client line:%d] close stream \n", workerId, line+1)
				return
				//close(jobDone)
			} else {
	...
	go func() {
		defer close(jobDone)
		<-ctx.Done()
		log.Printf("[Proxy_Verify][WorkerID: %d] Client Context Done", workerId)
		if err := ctx.Err(); err != nil {
			log.Println(err)
		}
	}()
...
func main() {
	...
	*/
	...
	buff_hello_request :=
		[]byte{0x0, 0x0, 0x3, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x18, 0xa, 0x0, 0x32, 0x5, 0x0, 0x0, 0xea, 0x61, 0x0, 0x0, 0x0, 0x0}
